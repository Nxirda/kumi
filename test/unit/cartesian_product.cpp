...
TTS_CASE("Check result::cartesian_product<Tuple...> behavior on named tuples")
{
  using namespace kumi::literals;
  using f1 = decltype("x"_m = 1     );
  using f2 = decltype("y"_m = 2.f   );
  using f3 = decltype("z"_m = 3.    );

  TTS_TYPE_IS ( (kumi::result::cartesian_product_t<kumi::tuple<f1,f2,f3>>)
              , (kumi::tuple<kumi::tuple<int>,kumi::tuple<float>,kumi::tuple<double>>)
              );

  TTS_TYPE_IS ( (kumi::result::cartesian_product_t<kumi::tuple<f1&,f2,f3 const&>>)
              , (kumi::tuple<kumi::tuple<int&>,kumi::tuple<float>,kumi::tuple<double const&>>)
              );

  TTS_TYPE_IS ( (kumi::result::cartesian_product_t<kumi::tuple<f1,f2,f3>, kumi::tuple<f1*,f2*>>)
              , (kumi::tuple< kumi::tuple<int, int*>  , kumi::tuple<float, int*>  , kumi::tuple<double, int*>
                            , kumi::tuple<int, float*>, kumi::tuple<float, float*>, kumi::tuple<double, float*>
                            >)
              );
...
TTS_CASE("Check cartesian_product() behavior with references on named tuples")
{
  using namespace kumi::literals;
  ...
  auto t0 = kumi::tuple<f0,f0&,f0 const&>{a,a,a};
  auto cp0 = kumi::cartesian_product(t0);
...
TTS_CASE("Check cartesian_product(ts...) behavior on name dtuples")
{
  using namespace std::literals;
  using namespace kumi::literals;

  auto t1 = kumi::make_tuple("x"_m = 1, "y"_m = 2ULL);
  auto t2 = kumi::make_tuple("x"_m = 1.2, "y"_m = 3.4f, "z"_m = 5.6);
  auto t3 = kumi::make_tuple("x"_m = "first"s, "y"_m = "second"s, "z"_m = "third"s, "t"_m = "fourth"s);
  auto cp = kumi::cartesian_product(t1,t2,t3);
