...
TTS_CASE("Check result::project<F,Tuple...> behavior")
{
  auto lambda = [](auto const& m) { return &m; };
  using func_t = decltype(lambda);

  TTS_TYPE_IS ( (kumi::result::project_t<func_t,kumi::tuple<char,short,int,double>, 0, 1, 2>)
              , (kumi::tuple<char const*,short const*,int const*>)
              );

  auto add = [](auto tuple) { return kumi::apply([](auto a, auto b){ return a + b; }, tuple); };
  using add_t = decltype(add);

  TTS_TYPE_IS ( (kumi::result::project_t<add_t, kumi::tuple<kumi::tuple<char, char>,
                                                        kumi::tuple<short,short>,
                                                        kumi::tuple<int, int>,
