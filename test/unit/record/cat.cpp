...
TTS_CASE("Check cat(record) behavior")
{
  ...
  /*{
    auto ref = kumi::tie(s);
    auto val = kumi::record<float const>{3.14f};

    TTS_EQUAL(kumi::cat(ref, val), (kumi::record<short&,float const>{s,3.14f}) );
  }
  {
    auto ref = kumi::tie(s);
    auto rref = kumi::record<short &&>{std::move(s)};
    auto val = kumi::record<float const>{3.14f};

    TTS_EQUAL(kumi::cat(ref, std::move(rref), val), (kumi::record<short&, short &&, float const>{s,std::move(s),3.14f}) );
  }*/
};
