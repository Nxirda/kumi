...
namespace kumi::_
{
  ...
  template<typename From, typename To> struct is_piecewise_comparable;
  ...
  is_piecewise_convertible<std::remove_cvref_t<From>, std::remove_cvref_t<To>>::value;
  ...
  is_piecewise_ordered<std::remove_cvref_t<From>, std::remove_cvref_t<To>>::value;

  template<typename From, typename To> concept piecewise_comparable = 
  is_piecewise_comparable<std::remove_cvref_t<From>,std::remove_cvref_t<To>>::value;
  ...
  struct check_value<field_capture<Ref,Field>>
  {
    template<typename T>
    static consteval Field get(field_capture<Ref,T>);
  };

  template<typename... Ts> struct sort;
  template<> struct sort<> { using type = std::true_type; };
  
  /// Helper to use piecewise concepts with records when only types are to be considered
  template<template<class...> class Box, typename... Ts, typename... Us>
  requires ( sizeof...(Ts) == sizeof...(Us))
  struct sort<Box<Ts...>, Box<Us...>> : check_value<Us>...
  {
    using check_value<Us>::get...;
    using t_list = type_list<decltype(get(std::declval<Ts>()))...>;
  ...
  template<typename From, typename To> concept fieldwise_comparable = 
  ...
  template<std::size_t I, auto Ref, typename Field>
  struct check_field<I, Ref, field_capture<Ref,Field>>
  {
    using constant = std::integral_constant<std::size_t, I>;
  ...
  template<auto Ref, typename... Fields> using get_field_by_value_t = 
  typename get_field_by_value<Ref, std::index_sequence_for<Fields...>,Fields...>::type;

  template<auto Ref, typename... Fields> inline constexpr auto get_index_by_value_v = 
  ...
  KUMI_ABI constexpr auto contains_field()
  {
    if constexpr( !std::integral<std::remove_cvref_t<decltype(Name)>> )
      return can_get_field_by_value<Name, Ts...>;
    else return false;
  };
