...
namespace kumi
{
  ...
  requires _::supports_apply<Function,T>
  {
    ...
    else return [&]<std::size_t...I>(std::index_sequence<I...>) -> decltype(auto)
    {
  ...
  namespace _
  {
      template<typename Function, record_type R>
      KUMI_ABI constexpr decltype(auto) apply_field(Function && f, R && r)
      {
          return [&]<std::size_t... I>(std::index_sequence<I...>) -> decltype(auto)
          {
            return invoke(KUMI_FWD(f), get<I>(KUMI_FWD(r))...);
