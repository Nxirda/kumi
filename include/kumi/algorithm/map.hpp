...
namespace kumi
{
             ...
             _::supports_call<Function, Tuple&&, Tuples&&...> )
  {
    ...
    {
      if constexpr ( record_type<base_t> )
      {
        using field_t = field_name<member_name_v<N, base_t>>;
  ...
  namespace result
  {
   ...
   //================================================================================================
  //! @ingroup transforms
  //! @brief Apply the Callable object f on each tuples' elements and their field names 
  //!
  //! Applies the given function to all the tuples passed as arguments along with their indexes  and
  //! stores the result in another tuple, keeping the original elements order.
  //!
  ...
  constexpr auto map_field(Function     f,Tuple  &&t0,Tuples &&...others)
  {
    ...
    else
    {
      auto const call = [&]<std::size_t N, typename... Ts>(index_t<N>, Ts &&... args)
      {
        constexpr auto name = member_name_v<N, base_t>; 
