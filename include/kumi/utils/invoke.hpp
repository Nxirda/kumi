...
namespace kumi
{
  ...
  KUMI_ABI constexpr decltype(auto) invoke_memptr( P C::* member, O && o, Ts &&... ts )
  {
    ...
    if constexpr ( std::is_member_object_pointer_v<callable_t> )
      return KUMI_FWD(ptr).*member;
    else 
      return (KUMI_FWD(ptr).*member)(KUMI_FWD(ts)...);
  };

  template<typename C, typename... Ts>
  KUMI_ABI constexpr decltype(auto) invoke(C && c, Ts &&... ts) 
  noexcept( std::is_nothrow_invocable<C,Ts...>::value )
  requires( std::is_invocable<C,Ts...>::value )
  {
    if constexpr ( std::is_member_pointer_v<std::decay_t<C>> )
      return invoke_memptr(c, KUMI_FWD(ts)...);
    else 
      return KUMI_FWD(c)(KUMI_FWD(ts)...);
  };

  template<typename R, typename C, typename... Ts>
  KUMI_ABI constexpr R invoke_r(C && c, Ts &&... ts)
  noexcept( std::is_nothrow_invocable_r<R,C,Ts...>::value )
